...
class GmailChannel(Channel):
    ...
    config_fields = [
        ConfigField("email", "Gmail address", placeholder="you@gmail.com"),
        ConfigField(
            "app_password", "App password", secret=True,
    ...
    def __init__(
        self, config: dict[str, str], *, smtp_factory: SmtpFactory = _default_smtp
    ) -> None:
    ...
    def send(self, message: OutboundMessage) -> DeliveryResult:
        ...
        try:
            with self._smtp_factory("smtp.gmail.com", 465) as server:
                server.login(self.email, self.app_password)
                server.send_message(msg)
        except smtplib.SMTPAuthenticationError as exc:
            raise ChannelError(
                "Gmail rejected the credentials — check the address and App Password."
            ) from exc
        except OSError as exc:
            raise ChannelError(f"Could not reach Gmail SMTP: {exc}") from exc
